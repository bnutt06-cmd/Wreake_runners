...
// Survives StoreProvider remounts. The provider was observed to remount on
// navigation; without this, each remount reset user/profile to null and
// briefly showed the logged-out state (the "Member" chip / admin lockout).
// By caching the last-known auth here and seeding React state from it, a
// remount starts already-authenticated instead of from scratch.
const authCache = {
  user: null,
  profile: null,
  authReady: false,
...
export function StoreProvider({ children }) {
  ...
  // Seed from the module cache so a remount keeps the user logged in.
  ...
  // arrived yet.
  const [authReady, setAuthReady] = useState(authCache.authReady);

  // Mirror auth state into the module cache so a provider remount can seed
  // from it synchronously (no logged-out flash on navigation).
  useEffect(() => {
    authCache.user = user;
    authCache.profile = profile;
    authCache.authReady = authReady;
  ...
  useEffect(() => {
    ...
    // This effect must run EXACTLY ONCE for the lifetime of the provider.
    // It sets up the auth listener and does the initial load. Listing the
    // loader callbacks as deps risked re-running it (which reset all state
    // and caused the profile to vanish on navigation). The loaders are
    // stable closures; we intentionally run this once on mount only.
