...
export function StoreProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange(async (event, session) => {
      console.log("[store] onAuthStateChange:", event);
      ...
      const u = session?.user || null;
      setUser(u);
      // Mark auth resolved immediately based on the session we already have
      // in hand — do NOT wait on the profile network call, which could hang
      // and leave admin pages stuck on "Loading...".
      if (active) setAuthReady(true);
      // Load the profile in the background; loadProfile preserves any
      // existing profile if this fails, so a transient error is harmless.
      const prof = await safe("loadProfile", () => loadProfile(u?.id));
      const canSeeAll = prof?.role === ROLES.ADMIN || prof?.role === ROLES.CONTENT_ADMIN;
      await Promise.all([
        safe("loadNews", () => loadNews(canSeeAll)),
