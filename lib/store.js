...
export function StoreProvider({ children, initialUser = null }) {
  const [supabase] = useState(() => createClient());

  // Seed user from the SERVER-PROVIDED initialUser. The root layout reads the
  // session from cookies (the source of truth) and passes it in, so the
  // client starts already-authenticated — no null/logged-out flash, no
  // "Member" chip on navigation. This is the core of the fix.
  ...
  // Seed readiness from the server-provided initialUser. If the server
  // already told us who the user is, we are NOT waiting to discover identity
  // — so loading starts false and auth is already "ready". This prevents the
  // hydration flicker where, on a fresh mount (e.g. via the key-prop remount
  // on identity change), these flags reset to their "still figuring it out"
  // defaults and briefly tripped the dashboard guard into showing Loading.
  const [loading, setLoading] = useState(!initialUser);
  // authReady: true once we've resolved identity. Because the server already
  // told us whether there's a user, we can consider auth "ready" as soon as
  // the (known) user's profile has loaded — or immediately if logged out.
  ...
  // Safe to make an access decision once auth has resolved AND either the
  // profile is loaded or we know there's no user.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Identity comes from the server via initialUser (seeded into `user`
        // state above). On an identity change, the key prop on StoreProvider
        // (in app/layout.js) tears down and rebuilds this whole provider, so
        // this effect re-runs fresh with the new initialUser — no separate
        // sync effect is needed, and the dependency array stays EMPTY to
        // avoid the loader-driven re-run loop that caused the original bug.
        // This effect loads the profile + bulk app data for the known user.
        const uid = initialUser?.id || null;
        const prof = uid ? await safe("loadProfile", () => loadProfile(uid)) : null;
        ...
        await Promise.all([
          ...
          safe("loadMyStandardsLogs", () => loadMyStandardsLogs(uid)),
          safe("loadClubEvents", loadClubEvents),
          safe("loadPinnedPost", loadPinnedPost),
        ]);
      } catch (e) {
        console.warn("[store] init: unexpected error", e);
      } finally {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange(async (event, session) => {
      // Tame the listener: it exists ONLY to react to real sign-in / sign-out
      // that happen DURING this session (e.g. logging in on the login page,
      // or clicking log out). It must never override the server-provided
      // identity on navigation noise.

      // Ignore token refreshes and the initial fire — the server already
      ...
      // A real sign-in during this session (login page).
      ...
      // Any other event (incl. null-session noise) is ignored — the
      // server-provided identity stands.
    });
  ...
  // ---- PROFILE GUARANTEE ----
  // Whenever we have a logged-in user but no profile in state, load it. This
  // is the definitive fix for admin pages stalling on "Loading...": the
  // access gate (accessResolved) needs the profile, and if any render path
  // (e.g. /admin/races, which uses useSearchParams and renders on a
  // different timing) reaches a state of user-but-no-profile, this guarantees
  // the profile gets fetched and the gate resolves. Covers every page at
  // once rather than patching them individually.
  useEffect(() => {
    if (user && !profile) {
      loadProfile(user.id);
    }
  ...
  return (
    <StoreContext.Provider
      value={{
        ...
        loadRaces, loadRaceRegs, loadStandardsLookup, loadMyStandardsLogs, loadProfile,
