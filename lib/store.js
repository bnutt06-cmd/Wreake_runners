...
export function StoreProvider({ children, initialUser = null }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // We already know the user from the server (initialUser, seeded into
        // `user`). No need to call getSession/getUser to discover identity —
        // that was the source of the "session? false" flash. Just load the
        // profile and data for the known user.
        const uid = initialUser?.id || null;
        const prof = await safe("loadProfile", () => loadProfile(uid));
