...
export function StoreProvider({ children, initialUser = null }) {
  ...
  // CRITICAL: useState(initialUser) only reads initialUser ONCE, on first
  // mount. On soft client-side navigation the server passes a fresh
  // initialUser prop, but useState ignores it — leaving the store stale.
  // This effect syncs the server-provided user into state whenever it
  // changes, so navigation always reflects the server's source-of-truth
  // identity. This is what makes server hydration actually work.
  useEffect(() => {
    setUser(initialUser);
    if (initialUser) {
      // Refresh the profile for the (possibly new) user.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Identity + profile are handled by the initialUser sync effect above.
        // This effect loads the bulk app data once on mount.
