...
export function StoreProvider({ children }) {
  ...
  const loadProfile = useCallback(async (uid) => {
    ...
    if (error) console.warn("[store] loadProfile failed", error);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Auth identity is now resolved (we know the profile, or know there's
        // no user). Mark ready BEFORE the slower data loads so the admin gate
        // can decide without waiting on every table.
        ...
        // ALWAYS set loading=false, even if everything above broke.
        if (active) { setLoading(false); setAuthReady(true); }
      }
    })();

    const { data: sub } = supabase.auth.onAuthStateChange(async (event, session) => {
      // Ignore TOKEN_REFRESHED, which fires periodically and would otherwise
      // trigger a full data reload on every refresh tick.
  ...
  return (
    <StoreContext.Provider
      value={{
        user, profile, role, loggedIn, isAdmin, isContentAdmin, canCreateNews, loading, authReady,
